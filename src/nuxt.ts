...
import { addImports, addTemplate, addVitePlugin, defineNuxtModule, isNuxt2, addPlugin, isNuxt3 } from '@nuxt/kit';
...
export default defineNuxtModule<Options & { composableName?: string }>({
	...
	async setup(options, nuxt) {
		...
		writeFileSync(PLUGIN_PATH, PLUGIN_CONTENTS.replace(
			'{{options}}',
			JSON.stringify(options.bemOptions, null, '\t'.repeat(indentationCount)),
		));
		...
		const composableFilename = `${PLUGIN_DIRECTORY}/use-bem.js`;
		const composablePath = addTemplate({
			...
			getContents: () => [
				...
				'',
				`export const useBem = useBemFactory(${JSON.stringify(options.bemOptions, null, '\t')});`,
			].join('\n'),
			write: true,
		}).dst;
